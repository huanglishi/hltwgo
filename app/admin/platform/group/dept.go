...
func Getlist(context *gin.Context) {
	menuList, _ := DB().Table("admin_platform_group").Order("weigh asc").Get()
...
func GetParentList(context *gin.Context) {
	menuList, _ := DB().Table("admin_platform_group").Fields("id,pid,name").Order("weigh asc").Get()
	menuList = GetMenuChildrenArray(menuList, 0)
...
func Add(context *gin.Context) {
	...
	if f_id == 0 {
		addId, err := DB().Table("admin_platform_group").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("admin_platform_group").
		...
		res, err := DB().Table("admin_platform_group").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func UpLock(context *gin.Context) {
	...
	res2, err := DB().Table("admin_platform_group").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func Del(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("admin_platform_group").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
