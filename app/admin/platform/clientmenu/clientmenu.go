...
func Getlist(context *gin.Context) {
	menuList, _ := DB().Table("client_auth_rule").Order("orderNo asc").Get()
...
func GetParentList(context *gin.Context) {
	menuList, _ := DB().Table("client_auth_rule").WhereIn("type", []interface{}{0, 1}).Fields("id,parentMenu,menuName").Order("orderNo asc").Get()
	menuList = GetMenuChildrenArray(menuList, 0)
...
func GetMenuList(context *gin.Context) {
	menuList, _ := DB().Table("client_auth_rule").Fields("id,parentMenu,menuName").Order("orderNo asc").Get()
	menuList = GetMenuChildrenArray(menuList, 0)
...
func Add(context *gin.Context) {
	...
	if f_id == 0 {
		addId, err := DB().Table("client_auth_rule").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加菜单失败", err)
		} else {
			if addId != 0 {
				DB().Table("client_auth_rule").
		...
		res, err := DB().Table("client_auth_rule").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func UpMenuLock(context *gin.Context) {
	...
	res2, err := DB().Table("client_auth_rule").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
...
func Del(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("client_auth_rule").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
