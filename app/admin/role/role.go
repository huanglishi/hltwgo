...
func Getlist(context *gin.Context) {
	...
	menuList, _ := DB().Table("admin_auth_role").WhereIn("id", all_role_id).Order("weigh asc").Get()
	menuList = GetTreeArray(menuList, 0, "")
	results.Success(context, "获取拥有权限列表", menuList, nil)
}
...
func GetParentList(context *gin.Context) {
	...
	role_ids := GetAllChilIds(role_id.([]interface{})) //批量获取子节点id
	all_role_id := mergeArr(role_id.([]interface{}), role_ids)
	menuList, _ := DB().Table("admin_auth_role").WhereIn("id", all_role_id).Fields("id,pid,name").Order("weigh asc").Get()
...
func GetAllList(context *gin.Context) {
	menuList, _ := DB().Table("admin_auth_role").Where("status", 0).Fields("id as value,pid,name as label").Order("weigh asc").Get()
...
func Add(context *gin.Context) {
	...
	if f_id == 0 {
		addId, err := DB().Table("admin_auth_role").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("admin_auth_role").
		...
		res, err := DB().Table("admin_auth_role").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func UpLock(context *gin.Context) {
	...
	res2, err := DB().Table("admin_auth_role").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func Del(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("admin_auth_role").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
