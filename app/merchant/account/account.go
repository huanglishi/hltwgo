...
func Getlist(context *gin.Context) {
	...
	MDB := DB().Table("merchant_user").Fields("id,status,name,username,avatar,mobile,email,remark,deptid,remark,createtime").Where("accountID", user.Accountid)
	if deptId != "0" {
	...
	if err != nil {
		...
		for _, val := range list {
			roleid, _ := DB().Table("merchant_auth_role_access").Where("uid", val["id"]).Pluck("role_id")
			rolename, _ := DB().Table("merchant_auth_role").WhereIn("id", roleid.([]interface{})).Pluck("name")
			val["rolename"] = rolename
			val["roleid"] = roleid
			depname, _ := DB().Table("merchant_auth_dept").Where("id", val["deptid"]).Value("name")
			val["depname"] = depname
		}
...
func appRoleAccess(roleids []interface{}, uid interface{}) {
	//批量提交
	DB().Table("merchant_auth_role_access").Where("uid", uid).Delete()
	...
	DB().Table("merchant_auth_role_access").Data(save_arr).Insert()
}
...
func UpAvatar(context *gin.Context) {
	...
	getuser, _ := context.Get("user")
	user := getuser.(*utils.UserClaims)
	res2, err := DB().Table("merchant_user").Where("id", user.ID).Data(map[string]interface{}{"avatar": parameter["url"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		userdata, err := DB().Table("merchant_user").Fields("id,username,name,avatar,status").Where("id", user.ID).First()
		if err != nil {
		...
		//获取用户权限
		results.Success(context, "更新头像成功！", map[string]interface{}{
			"userId":   userdata["id"],
...
func ChangePwd(context *gin.Context) {
	...
	getuser, _ := context.Get("user")
	user := getuser.(*utils.UserClaims)
	userdata, _ := DB().Table("merchant_user").Where("id", user.ID).Fields("id,password,salt").First()
	...
	if pass != userdata["password"].(string) {
		...
		res2, err := DB().Table("merchant_user").Where("id", user.ID).Data(map[string]interface{}{"password": newpass}).Update()
		if err != nil {
...
func Del(context *gin.Context) {
	...
	if err != nil {
		...
		//并删除角色权限
		DB().Table("merchant_auth_role_access").WhereIn("uid", ids.([]interface{})).Delete()
...
func GetLoginLogList(context *gin.Context) {
	...
	pageNo, _ := strconv.Atoi(page)
	pageSize, _ := strconv.Atoi(_pageSize)
	list, _ := DB().Table("login_logs").Where("uid", userID).Where("type", 2).Limit(pageSize).Page(pageNo).Order("createtime desc").Get()
	var totalCount int64
	totalCount, _ = DB().Table("login_logs").Where("uid", userID).Where("type", 2).Count()
...
func GetAccount(context *gin.Context) {
	...
	getuser, _ := context.Get("user")
	user := getuser.(*utils.UserClaims)
	data, _ := DB().Table("merchant_user").Where("id", user.ID).First()
