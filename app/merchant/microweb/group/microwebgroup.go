...
func Getlist(context *gin.Context) {
	...
	menuList, _ := DB().Table("merchant_micweb_group").Where("accountID", user.Accountid).Order("weigh asc").Get()
...
func GetParentList(context *gin.Context) {
	...
	getuser, _ := context.Get("user")
	user := getuser.(*utils.UserClaims)
	menuList, _ := DB().Table("merchant_micweb_group").Fields("id,pid,name").Where("accountID", user.Accountid).Order("weigh asc").Get()
...
func GetCateList(context *gin.Context) {
	...
	getuser, _ := context.Get("user")
	user := getuser.(*utils.UserClaims)
	menuList, _ := DB().Table("merchant_micweb_group").Fields("id,pid,name,remark,status,weigh").Where("accountID", user.Accountid).OrWhere("type", 0).Order("weigh asc").Get()
	menuList = GetMenuChildrenArray(menuList, 0)
...
func GetGroupTree(context *gin.Context) {
	...
	MDB := DB().Table("merchant_micweb_group")
	if keyword != "" {
		MDB = MDB.Where("name", "like", "%"+keyword+"%")
	}
	list, _ := MDB.Fields("id,pid,name").Where("accountID", user.Accountid).OrWhere("type", 0).Order("weigh asc").Get()
	// menuList = GetMenuChildrenArray(menuList, 0)
...
func Add(context *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := DB().Table("merchant_micweb_group").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("merchant_micweb_group").
		...
		res, err := DB().Table("merchant_micweb_group").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func UpLock(context *gin.Context) {
	...
	res2, err := DB().Table("merchant_micweb_group").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func UpGrouppid(context *gin.Context) {
	...
	var ids_arr []interface{}
	json.Unmarshal([]byte(b_ids), &ids_arr)
	res2, err := DB().Table("merchant_micweb_group").WhereIn("id", ids_arr).Data(map[string]interface{}{"pid": parameter["pid"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func Del(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("merchant_micweb_group").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
