...
func Lonin(context *gin.Context) {
	...
	userdata, err := DB().Table("merchant_user").Fields("id,accountID,password,salt,name,avatar").Where("username", username).First()
	...
	DB().Table("merchant_user").Where("id", userdata["id"]).Data(map[string]interface{}{"status": 1, "lastLoginTime": time.Now().Unix(), "lastLoginIp": utils.GetRequestIP(context)}).Update()
	//登录日志
	DB().Table("login_logs").
		Data(map[string]interface{}{"type": 2, "uid": userdata["id"], "out_in": "in",
