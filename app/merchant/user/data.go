...
func GetInfo(context *gin.Context) {
	...
	userdata, err := DB().Table("merchant_user").Fields("id,accountID,username,name,avatar,status").Where("id", user.ID).First()
	if err != nil {
...
func QueryParam(context *gin.Context) {
	...
	if utils.In_array(access_all, data_access_ids) { //全部
		_res, _err := DB().Table("merchant_user").Fields("id, name,username,telephone,lastLoginIp,lastLoginTime,status,valid_time").Page(pageNo).Limit(pageSize).Order("id desc").Get()
		res = _res
		err = _err
		...
		_res, _err := DB().Table("merchant_user").Where("uid", user.ID).Fields("id, name,username,telephone,lastLoginIp,lastLoginTime,status,valid_time").Page(pageNo).Limit(pageSize).Order("id desc").Get()
		res = _res
		err = _err
	}
	if err != nil {
...
func Changepassword(context *gin.Context) {
	...
	user := getuser.(*utils.UserClaims)
	userdata, err := DB().Table("merchant_user").Where("id", user.ID).Fields("password,salt").First()
	if err != nil {
