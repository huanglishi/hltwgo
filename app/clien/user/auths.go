...
func GetMenuList(context *gin.Context) {
	...
	MDB := DB().Table("client_auth_rule").Fields("id,title,menuName,orderNo,type,parentMenu,icon,routePath,routeName,component,redirect,permission,isExt,keepalive,hideMenu,hideBreadcrumb,hideChildrenInMenu,currentActiveMenu,hideTab")
	//获取用户权限
	role_id, _ := DB().Table("client_auth_role_access").Where("uid", user.ID).Pluck("role_id")
	if role_id == nil {
		results.Failed(context, "您没有使用权限", ok)
		return
	}
	menu_ids, _ := DB().Table("client_auth_role").WhereIn("id", role_id.([]interface{})).Pluck("rules")
	getmenus := ArrayMerge(menu_ids.([]interface{}))
	nemu_list, err := MDB.
		// WhereIn("id", getmenus).
		Where("status", 0).WhereIn("type", []interface{}{0, 1}).Order("orderNo asc").Get()
	if err != nil {
		results.Failed(context, "查找菜单列表失败！", err)
		return
	}
	rulenum := GetMenuArray(nemu_list, 0)
	results.Success(context, "获取用户菜单列表!", rulenum, getmenus)
}
...
func GetRoleMenuList(context *gin.Context) {
	...
	if id == zero || pid == zero { //获取本账号所拥有的权限
		...
		user := getuser.(*utils.UserClaims)
		role_id, _ := DB().Table("client_auth_role_access").Where("uid", user.ID).Pluck("role_id")
		menu_id, _ := DB().Table("client_auth_role").WhereIn("id", role_id.([]interface{})).Pluck("rules")
		...
		//获取用户权限
		pid, _ := DB().Table("client_auth_role").Where("id", id).Value("pid") //获取父级权限
		menu_id_str, _ := DB().Table("client_auth_role").Where("id", pid).Value("rules")
