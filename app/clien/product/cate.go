...
func GetCateList(context *gin.Context) {
	...
	wheremap := DB().Table("client_product_cate").Where("cuid", user.ClientID)
	wheremap2 := DB().Table("client_product_cate").Where("cuid", user.ClientID)
	if status != "0" {
	...
	if err != nil {
		...
		for _, val := range list {
			//上一级
			pidname, _ := DB().Table("client_product_cate").Where("id", val["pid"]).Value("name")
			if pidname != nil {
				val["pidname"] = pidname
			} else {
				val["pidname"] = "无"
...
func SaveCate(context *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := DB().Table("client_product_cate").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("client_product_cate").
		...
		res, err := DB().Table("client_product_cate").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func GetFormCateList(context *gin.Context) {
	...
	MDB := DB().Table("client_product_cate")
	if keyword != "" {
		MDB = MDB.Where("name", "like", "%"+keyword+"%")
	}
	list, _ := MDB.Fields("id,pid,name").Where("cuid", user.ID).Order("weigh desc").Get()
	rulenum := GetTreeArray(list, 0, "")
	list_text := GetTreeList_txt(rulenum, "name")
...
func DelCate(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("client_product_cate").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
func UpCateStatus(context *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := DB().Table("client_product_cate").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
