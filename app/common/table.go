...
// @Param  ids int  true  "表单ID"
// @Router /common/tabl/weigh/ [get]
func Weigh(context *gin.Context) {
	//获取post传过来的data
	...
	if _, ok := parameter["pid"]; ok {
		...
		list_id, _ := DB().Table(tablename).WhereIn(prikey, ids_arr).Where("pid", pid).Pluck("id")
	...
	list, _ := DB().Table(tablename).WhereIn(prikey, winids).Fields(prikey + "," + field).Order(field + " " + orderway).Get()
	var sour []int64
	...
	for k, v := range temp {
		...
		DB().Table(tablename).Where(prikey, v).Data(map[string]interface{}{field: weighdata[offset]}).Update()
	}
